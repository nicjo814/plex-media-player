...
int InputCECWorker::CecCommand(void *cbParam, const cec_command command)
{
  ...
  switch(command.opcode)
  {
    ...
    case CEC_OPCODE_VENDOR_REMOTE_BUTTON_DOWN:
      ...
      if (cec->m_verboseLogging)
        QLOG_DEBUG() << "CecCommand button DOWN " << cec->getCommandParamsList(command);
    ...
    case CEC_OPCODE_USER_CONTROL_PRESSED:
      if (cec->m_verboseLogging)
        QLOG_DEBUG() << "CecCommand control PRESSED " << cec->getCommandParamsList(command);

      cmdString = cec->getCommandString((cec_user_control_code)command.parameters[0]);

      if (!cmdString.isEmpty())
        cec->sendReceivedInput(CEC_INPUT_NAME, cmdString, true);
      break;

    case CEC_OPCODE_USER_CONTROL_RELEASE:
    case CEC_OPCODE_VENDOR_REMOTE_BUTTON_UP:
       QLOG_DEBUG() << "CecCommand button UP " << cec->getCommandParamsList(command);

       cmdString = cec->getCommandString((cec_user_control_code)command.parameters[0]);

       if (!cmdString.isEmpty())
         cec->sendReceivedInput(CEC_INPUT_NAME, cmdString, false);
      break;
